...
- Automated VyOS installation via boot commands following the official installation process:
  ...
  5. Run installation with proper prompts:
     ...
     - Set password
     - "S" for Serial console (recommended for cloud) or "K" for KVM
...
- `console_type`: Console type - "S" for Serial (default, cloud), "K" for KVM
...
7. **Installation Prompts**:
   ...
   - Select Serial console ("S" - recommended for cloud environments)
...
This build uses **serial console** (ttyS0) as the default, which is the recommended choice for cloud images:

**Why Serial Console?**
- ✅ **Cloud compatibility**: All major cloud providers (AWS, Azure, GCP, OpenStack) use serial console
- ✅ **Automation friendly**: Works with cloud-init and automated provisioning tools
- ✅ **Remote access**: Access via `virsh console` or cloud provider console
- ✅ **Out-of-band management**: Compatible with IPMI/BMC for bare-metal
- ✅ **Universal support**: Works across all hypervisors and platforms

**When to use KVM console?**
